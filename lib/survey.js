...
const DESIGNS = [
  ...
  },
  {
    ...
    folder: "iterace-06-agent-dashboard",
    previewPath: "/designs/iterace-06-agent-dashboard/",
...
function validateSubmission(payload) {
  ...
  const scoresInput = Array.isArray(input.scores) ? input.scores : [];
  ...
  for (const item of scoresInput) {
    const scoreInput = item && typeof item === "object" ? item : {};
    const designId = typeof scoreInput.designId === "string" ? scoreInput.designId.trim() : "";
    ...
    seen.add(designId);
    const score = normalizeScore(scoreInput.score);
    if (score === null) {
    ...
    scores.push({
      designId,
      score,
      note: sanitizeNote(scoreInput.note)
    });
  }
  ...
  const requestedOrder = Array.isArray(input.displayOrder) ? input.displayOrder : [];
